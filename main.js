/* Note Calendar Plus (fork of Note Calendar v1.3.1) - built with esbuild */
...
var require_lunar_javascript = __commonJS({
  "node_modules/lunar-javascript/index.js"(exports, module2) {
    var { Solar: Solar2, Lunar: Lunar2, Foto, Tao, NineStar, EightChar, SolarWeek, SolarMonth, SolarSeason, SolarHalfYear, SolarYear, LunarMonth, LunarYear, LunarTime, ShouXingUtil, SolarUtil, LunarUtil, FotoUtil, TaoUtil, HolidayUtil: HolidayUtil2, NineStarUtil, I18n } = require_lunar();
    module2.exports = {
      Solar: Solar2,
      Lunar: Lunar2,
      Foto,
...
var DEFAULT_SETTINGS = {
  startOfWeek: 0,
  /** 素观 v1.8.0：主题色默认近黑 #1a1a1a（灰度主导），周末文字默认灰 #767676（去红色系） */
  weekendColor: "#767676",
  themeColor: "#1a1a1a",
...
var CalendarModel = class {
  constructor(settings = {}) {
    ...
    this.weekendColor = settings.weekendColor || "#767676";
    this.themeColor = settings.themeColor || "#1a1a1a";
  ...
  getCalendarData() {
    ...
    for (let i = firstDay - 1; i >= 0; i--) {
      ...
      calendarDays.push({ day, isCurrentMonth: false, date, weekNumber: null });
    }
    ...
    for (let i = 1; i <= remainingCells; i++) {
      ...
      calendarDays.push({ day: i, isCurrentMonth: false, date, weekNumber: null });
    }
    return calendarDays;
  }
...
var VIEW_TYPE_CALENDAR = "kumoo-calendar-view";
function extColor(_ext) {
  return "#f0f0f0";
}
var CalendarView = class extends import_obsidian.ItemView {
  constructor(leaf, plugin) {
    ...
    this.todayBtn = null;
    this.notesList = null;
    this.lunarTitleEl = null;
    // 文件活动筛选状态（会话内有效，不持久化）
    this.boardFilter = "";
    this.typeFilter = "";
    // 相关笔记摘要区异步渲染令牌：日期切换后旧结果丢弃
    this.planNotesToken = 0;
    this.plugin = plugin;
  ...
  async onOpen() {
    this.contentEl.addClass("kumoo-calendar-view");
    this.createCalendarView();
    setTimeout(() => {
      const hasData = this.model.noteCache && Object.keys(this.model.noteCache).length > 0 || this.plugin.pendingNoteCache && Object.keys(this.plugin.pendingNoteCache).length > 0;
      if (!hasData) {
  ...
  createHeader() {
    ...
    scanBtn.className = "calendar-btn calendar-nav-btn";
    scanBtn.textContent = "\u27F3";
    scanBtn.title = "\u91CD\u65B0\u626B\u63CF\u6587\u4EF6";
    scanBtn.onclick = async () => {
      new import_obsidian.Notice("\u6B63\u5728\u626B\u63CF\u6587\u4EF6\u2026");
      await this.plugin.scanNotes(true);
      new import_obsidian.Notice("\u626B\u63CF\u5B8C\u6210");
    };
    rightNavGroup.appendChild(scanBtn);
    const searchBtn = document.createElement("button");
    searchBtn.className = "calendar-btn calendar-nav-btn";
    searchBtn.textContent = "\u2315";
    searchBtn.title = "\u641C\u7D22\u6587\u4EF6\u5386\u53F2";
    searchBtn.onclick = () => {
      new FileHistoryModal(this.app, this).open();
    };
    rightNavGroup.appendChild(searchBtn);
  ...
  updateGrid() {
    ...
    weekdays.forEach((day, index) => {
      ...
      const isWeekend = this.model.startOfWeek === 0 && (index === 0 || index === 6) || this.model.startOfWeek === 1 && (index === 5 || index === 6);
      if (isWeekend)
    ...
    calendarData.forEach((dayData) => {
      ...
      if (this.model.showLunarDate) {
        ...
        lunarDayText.textContent = dayInChinese === "\u521D\u4E00" ? lunarDay.getMonthInChinese() + "\u6708" : dayInChinese;
      ...
      if (this.model.showLunarFestivals) {
        ...
        if (lunarFestivals && lunarFestivals.length > 0)
          festivalTexts.push(lunarFestivals[0]);
      }
      if (this.model.showSolarFestivals) {
        const solarFestivals = solarDay.getFestivals();
        if (solarFestivals && solarFestivals.length > 0)
          festivalTexts.push(solarFestivals[0]);
      }
      if (this.model.showJieQi) {
        const jieQi = lunarDay.getJieQi();
        if (jieQi)
          festivalTexts.push(jieQi);
      }
      if (festivalTexts.length > 0) {
        const festivalEl = document.createElement("div");
        festivalEl.className = "calendar-day-festival";
        festivalEl.textContent = festivalTexts.join(" \xB7 ");
        dayCell.appendChild(festivalEl);
      }
      if (holiday && this.model.showHolidayMarker) {
        const marker = document.createElement("div");
        marker.className = "calendar-holiday-marker";
        if (holiday.isWork()) {
          marker.classList.add("calendar-holiday-work");
          marker.textContent = "\u73ED";
        } else {
          marker.classList.add("calendar-holiday-rest");
          marker.textContent = "\u4F11";
        }
        dayCell.appendChild(marker);
      }
      const dateStr = this.model.formatDate(dayData.date);
      const activities = this.model.getNotesForDate(dateStr);
      const hasPlan = this.hasPlanNoteOn(dayData.date);
      if (activities.length > 0 || hasPlan) {
        if (dayData.isToday)
          dayCell.classList.add("calendar-day-today-with-dots");
        const dotsContainer = document.createElement("div");
        dotsContainer.className = "calendar-note-dots";
        const hasCreated = activities.some((n) => n.type === "created");
        const hasUpdated = activities.some((n) => n.type === "updated");
        const hasDeleted = activities.some((n) => n.type === "deleted");
        if (hasCreated) {
          const dot = document.createElement("div");
          dot.className = "calendar-note-dot calendar-note-dot-created";
          dotsContainer.appendChild(dot);
        }
        if (hasUpdated) {
          const dot = document.createElement("div");
          dot.className = "calendar-note-dot calendar-note-dot-updated";
          dotsContainer.appendChild(dot);
        }
        if (hasDeleted) {
          const dot = document.createElement("div");
          dot.className = "calendar-note-dot calendar-note-dot-deleted";
          dotsContainer.appendChild(dot);
        }
        if (hasPlan) {
          const dot = document.createElement("div");
          dot.className = "calendar-note-dot calendar-note-dot-plan";
          dotsContainer.appendChild(dot);
        }
        dayCell.appendChild(dotsContainer);
      }
      dayCell.onclick = () => {
        this.model.selectDate(dayData.date.getFullYear(), dayData.date.getMonth() + 1, dayData.date.getDate());
        this.render();
      };
  ...
  updateNotesList() {
    ...
    headerRow.className = "calendar-notes-header-row";
    const btnGroup = this.createNoteButtonsGroup();
    btnGroup.className = "calendar-create-btn-group calendar-btn-group-inline";
    ...
    titleContainer.appendChild(headerRow);
    const activities = this.model.getNotesForDate(dateStr);
    const boards = [...new Set(activities.map((a) => this.boardOf(a.path)))].sort();
    if (this.boardFilter && !boards.includes(this.boardFilter))
      this.boardFilter = "";
    const boardFiltered = activities.filter((a) => !this.boardFilter || this.boardOf(a.path) === this.boardFilter);
    const createdCount = boardFiltered.filter((n) => n.type === "created").length;
    const updatedCount = boardFiltered.filter((n) => n.type === "updated").length;
    const deletedCount = boardFiltered.filter((n) => n.type === "deleted").length;
    const filtered = boardFiltered.filter((a) => !this.typeFilter || a.type === this.typeFilter);
    if (this.model.showLunarDate) {
      try {
    ...
    if (activities.length > 0) {
      ...
      bottomRow.className = "calendar-notes-bottom-row";
      const filterRow = document.createElement("div");
      filterRow.className = "calendar-notes-filter";
      filterRow.appendChild(this.createFilterSelect(
        "\u5168\u90E8\u677F\u5757",
        boards.map((b) => ({ value: b, text: b })),
        this.boardFilter,
        (v) => {
          this.boardFilter = v;
      ...
      bottomRow.appendChild(filterRow);
      const statsSpan = document.createElement("span");
      statsSpan.className = "calendar-notes-stats";
      const totalEl = document.createElement("span");
      totalEl.className = "calendar-notes-stats-total";
      totalEl.textContent = `\u5171 ${boardFiltered.length} \u9879 \xB7 `;
      statsSpan.appendChild(totalEl);
      const mkToggle = (label, count, type) => {
        const el = document.createElement("span");
        el.className = "calendar-notes-stats-toggle";
        if (this.typeFilter === type)
          el.classList.add("calendar-notes-stats-toggle-active");
        el.textContent = `${label} ${count}`;
        el.title = this.typeFilter === type ? "\u53D6\u6D88\u7B5B\u9009" : `\u53EA\u770B${label}`;
        el.onclick = () => {
      ...
      statsSpan.appendChild(mkToggle("\u65B0\u5EFA", createdCount, "created"));
      statsSpan.appendChild(mkToggle("\u66F4\u65B0", updatedCount, "updated"));
      if (deletedCount > 0 || this.typeFilter === "deleted") {
        statsSpan.appendChild(mkToggle("\u5220\u9664", deletedCount, "deleted"));
      }
    ...
    if (filtered.length > 0) {
      ...
      sorted.forEach((note) => {
        ...
        extBadge.className = "calendar-note-item-ext";
        extBadge.textContent = note.ext || "file";
        extBadge.style.backgroundColor = extColor(note.ext || "file");
        noteItem.appendChild(extBadge);
        ...
        rightCol.className = "calendar-note-item-right";
        const tag = document.createElement("span");
        tag.className = note.type === "created" ? "calendar-note-item-tag calendar-note-tag-created" : note.type === "deleted" ? "calendar-note-item-tag calendar-note-tag-deleted" : "calendar-note-item-tag calendar-note-tag-updated";
        tag.textContent = note.type === "created" ? "\u65B0\u5EFA" : note.type === "deleted" ? "\u5220\u9664" : "\u66F4\u65B0";
        rightCol.appendChild(tag);
        const timeEl = document.createElement("div");
        timeEl.className = "calendar-note-item-time";
        const d = new Date(note.updatedAt);
        timeEl.textContent = `${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
        rightCol.appendChild(timeEl);
        noteItem.appendChild(rightCol);
        noteItem.onclick = async () => {
          ...
          if (file instanceof import_obsidian.TFile) {
            ...
            new import_obsidian.Notice("\u8BE5\u6587\u4EF6\u5DF2\u5220\u9664");
          }
        };
      ...
      emptyEl.textContent = "\u65E0\u7B26\u5408\u6761\u4EF6\u7684\u8BB0\u5F55";
      notesContainer.appendChild(emptyEl);
    } else {
      const emptyEl = document.createElement("div");
      emptyEl.className = "calendar-notes-empty";
      emptyEl.textContent = "\u8BE5\u65E5\u671F\u6682\u65E0\u6587\u4EF6\u6D3B\u52A8";
      notesContainer.appendChild(emptyEl);
    }
    notesList.appendChild(notesContainer);
    this.renderPlanNotes(notesList);
  }
  /**
   * 计划笔记候选：某日期对应的日记/周记/月度/季度/年度笔记路径。
   * 归属逻辑：日记=当天，周记=当天所在周，月度=当天所在月，季度=当天所在季，年度=当天所在年。
   * 与日期格「计划点」标记共用，保证日历标记与摘要区内容一致。
   */
  planCandidates(date) {
    return [
      { typeName: "\u65E5\u8BB0", title: this.generateNoteTitle("daily", date), folderPath: this.folderPathForType("daily") },
      { typeName: "\u5468\u8BB0", title: this.generateNoteTitle("weekly", date), folderPath: this.folderPathForType("weekly") },
      { typeName: "\u6708\u5EA6", title: this.generateNoteTitle("monthly", date), folderPath: this.folderPathForType("monthly") },
      { typeName: "\u5B63\u5EA6", title: this.generateNoteTitle("quarterly", date), folderPath: this.folderPathForType("quarterly") },
      { typeName: "\u5E74\u5EA6", title: this.generateNoteTitle("yearly", date), folderPath: this.folderPathForType("yearly") }
    ];
  }
  /** 该日期是否存在任一计划笔记（日期格琥珀点信号源） */
  hasPlanNoteOn(date) {
    for (const c of this.planCandidates(date)) {
      const filePath = c.folderPath ? `${c.folderPath}/${c.title}.md` : `${c.title}.md`;
  ...
  /**
   * 相关笔记摘要区：选中日期对应的日记/周记/月度/季度/年度笔记
   * 存在则读取缩略内容展示，点击跳转笔记本身。
   */
  async renderPlanNotes(notesList) {
    const selectedDate = this.model.selectedDate;
    if (!selectedDate)
      return;
    const token = this.planNotesToken;
    const candidates = this.planCandidates(selectedDate);
    const found = [];
    for (const c of candidates) {
      ...
      if (!(f instanceof import_obsidian.TFile))
        continue;
      const snippet = await this.noteSnippet(f);
      if (token !== this.planNotesToken)
        return;
      found.push({ typeName: c.typeName, file: f, snippet });
    }
    if (found.length === 0 || token !== this.planNotesToken)
      return;
    const section = document.createElement("div");
    section.className = "calendar-plan-notes";
    found.forEach(({ typeName, file, snippet }) => {
      const item = document.createElement("div");
      item.className = "calendar-plan-note";
      const typeBadge = document.createElement("span");
      typeBadge.className = "calendar-plan-note-type";
      typeBadge.textContent = typeName;
      item.appendChild(typeBadge);
      const body = document.createElement("div");
      body.className = "calendar-plan-note-body";
      const titleEl = document.createElement("div");
      titleEl.className = "calendar-plan-note-title";
      titleEl.textContent = file.basename;
      body.appendChild(titleEl);
      const snippetEl = document.createElement("div");
      snippetEl.className = "calendar-plan-note-snippet";
      snippetEl.textContent = snippet || "\uFF08\u7A7A\u767D\u7B14\u8BB0\uFF09";
    ...
    notesList.insertBefore(section, notesList.lastChild);
  }
  /** 读取笔记摘要：去 frontmatter 与标题行，压缩空白，取前 maxLen 字 */
  async noteSnippet(file, maxLen = 100) {
    try {
  ...
  /** 板块名：路径第一段目录；根目录文件归「未分类」 */
  boardOf(path) {
    const idx = path.indexOf("/");
    return idx > 0 ? path.slice(0, idx) : "\u672A\u5206\u7C7B";
  }
  /** 构建一个筛选下拉（首个选项为占位/全部） */
  createFilterSelect(placeholder, options, current, onchange) {
    const sel = document.createElement("select");
    sel.className = "calendar-filter-select";
  ...
  createNoteButtonsGroup() {
    ...
    const buttons = [
      { text: "+", title: "\u65E5\u8BB0\uFF1A\u5DF2\u521B\u5EFA\u5219\u6253\u5F00\uFF0C\u672A\u521B\u5EFA\u5219\u65B0\u5EFA", type: "daily" },
      { text: "\u5468", title: "\u5468\u8BB0\uFF1A\u5DF2\u521B\u5EFA\u5219\u6253\u5F00\uFF0C\u672A\u521B\u5EFA\u5219\u65B0\u5EFA", type: "weekly" },
      { text: "\u6708", title: "\u6708\u5EA6\u7B14\u8BB0\uFF1A\u5DF2\u521B\u5EFA\u5219\u6253\u5F00\uFF0C\u672A\u521B\u5EFA\u5219\u65B0\u5EFA", type: "monthly" },
      { text: "\u5B63", title: "\u5B63\u5EA6\u7B14\u8BB0\uFF1A\u5DF2\u521B\u5EFA\u5219\u6253\u5F00\uFF0C\u672A\u521B\u5EFA\u5219\u65B0\u5EFA", type: "quarterly" },
      { text: "\u5E74", title: "\u5E74\u5EA6\u7B14\u8BB0\uFF1A\u5DF2\u521B\u5EFA\u5219\u6253\u5F00\uFF0C\u672A\u521B\u5EFA\u5219\u65B0\u5EFA", type: "yearly" }
    ];
    buttons.forEach(({ text, title, type }) => {
      ...
      btn.title = title;
      btn.onclick = () => this.handleNoteButton(type);
      group.appendChild(btn);
    });
    return group;
  }
  /** 按钮智能行为：目标笔记已存在 → 直接打开；不存在 → 确认创建（含路径设置）后打开 */
  async handleNoteButton(type = "daily") {
    const selectedDate = this.model.selectedDate;
    if (!selectedDate)
      return;
    const title = this.generateNoteTitle(type, selectedDate);
    const folderPath = this.folderPathForType(type);
    ...
    this.showCreateNoteDialog(type, title, folderPath, filePath);
  }
  folderPathForType(type) {
    switch (type) {
      case "daily":
        return this.model.dailyFolderPath || "";
      case "weekly":
        return this.model.weeklyFolderPath || "";
      case "monthly":
        return this.model.monthlyFolderPath || "";
      case "quarterly":
        return this.model.quarterlyFolderPath || "";
      case "yearly":
        return this.model.yearlyFolderPath || "";
      default:
        return "";
    }
  }
  showCreateNoteDialog(type = "daily", presetTitle, presetFolder, presetPath) {
    const selectedDate = this.model.selectedDate;
    if (!selectedDate)
      return;
    const defaultTitle = presetTitle != null ? presetTitle : this.generateNoteTitle(type, selectedDate);
    const defaultFolder = presetFolder != null ? presetFolder : this.folderPathForType(type);
    const typeName = type === "daily" ? "\u65E5\u8BB0" : type === "weekly" ? "\u5468\u8BB0" : type === "monthly" ? "\u6708\u5EA6\u7B14\u8BB0" : type === "quarterly" ? "\u5B63\u5EA6\u7B14\u8BB0" : "\u5E74\u5EA6\u7B14\u8BB0";
    const modal = new import_obsidian.Modal(this.app);
    modal.titleEl.textContent = `\u521B\u5EFA${typeName}`;
    ...
    titleView.textContent = defaultTitle;
    titleView.style.padding = "8px";
    titleView.style.border = "1px solid var(--calendar-border)";
    titleView.style.borderRadius = "4px";
    titleView.style.backgroundColor = "var(--calendar-bg-subtle)";
    titleView.style.color = "var(--calendar-text)";
    titleDiv.appendChild(titleView);
    ...
    pathHint.style.color = "var(--calendar-text-muted)";
    pathHint.textContent = `\u5C06\u521B\u5EFA: ${presetPath != null ? presetPath : defaultFolder ? `${defaultFolder}/${defaultTitle}.md` : `${defaultTitle}.md`}`;
    ...
    confirmBtn.onclick = async () => {
      var _a;
      const title = ((_a = titleView.textContent) == null ? void 0 : _a.trim()) || "";
      const folderPath = folderInput.value.trim();
      if (!title) {
        new import_obsidian.Notice("\u7B14\u8BB0\u6807\u9898\u4E0D\u80FD\u4E3A\u7A7A");
        return;
      }
      if (folderPath !== this.folderPathForType(type)) {
        const patch = {};
        patch[`${type}FolderPath`] = folderPath;
        this.model[`${type}FolderPath`] = folderPath;
        await this.plugin.updateSettings(patch);
      }
      const created = await this.createNote(title, folderPath, type);
      modal.close();
      if (created) {
        const leaf = this.app.workspace.getLeaf(false);
  ...
  async createNote(title, folderPath, type) {
    try {
      ...
      if (folderPath) {
        ...
        filePath = `${normalized}${title}.md`;
      } else {
        filePath = `${title}.md`;
      }
      const existingFile = this.app.vault.getAbstractFileByPath(filePath);
      if (existingFile instanceof import_obsidian.TFile) {
        new import_obsidian.Notice("\u8BE5\u7B14\u8BB0\u5DF2\u5B58\u5728");
        return existingFile;
      }
      if (folderPath) {
      ...
      this.plugin.queueIncrement(filePath, "create");
      return file;
    } catch (error) {
      console.error("[KumooCalendar] \u521B\u5EFA\u7B14\u8BB0\u5931\u8D25:", error);
  ...
  updateTodayButtonState() {
    ...
    const hasClass = todayBtn.classList.contains("calendar-today-btn-selected");
    if (isTodaySelected && !hasClass)
      todayBtn.classList.add("calendar-today-btn-selected");
    else if (!isTodaySelected && hasClass)
      todayBtn.classList.remove("calendar-today-btn-selected");
  }
  render() {
    if (!this.model.noteCache || Object.keys(this.model.noteCache).length === 0) {
      const pending = this.plugin.pendingNoteCache;
      if (pending && Object.keys(pending).length > 0) {
        this.model.noteCache = pending;
      }
    }
    if (!this.container)
      return;
    ...
    if (this.titleEl && this.lunarTitleEl) {
      ...
      if (this.model.showLunarDate) {
        try {
          ...
          this.lunarTitleEl.textContent = lunarDay.getYearInGanZhi() + lunarDay.getYearShengXiao() + "\u5E74 " + lunarDay.getMonthInChinese() + "\u6708";
        } catch (e) {
...
var FileHistoryModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    contentEl.empty();
    contentEl.addClass("calendar-fh-modal");
    contentEl.createEl("h3", { text: "\u6587\u4EF6\u5386\u53F2" });
    const input = contentEl.createEl("input");
    input.type = "text";
    input.className = "calendar-fh-input";
    input.placeholder = "\u8F93\u5165\u6587\u4EF6\u540D\u641C\u7D22\u2026\uFF08\u542B\u5DF2\u5220\u9664\u6587\u4EF6\uFF09";
    const resultBox = contentEl.createDiv("calendar-fh-results");
    const timelineBox = contentEl.createDiv("calendar-fh-timeline");
    const buildIndex = () => {
      const map = /* @__PURE__ */ new Map();
      const nc = this.view.model.noteCache;
      for (const dateStr in nc) {
        for (const n of nc[dateStr]) {
          if (!map.has(n.path))
            map.set(n.path, []);
    ...
    const showTimeline = (path, entries) => {
      var _a;
      resultBox.empty();
      timelineBox.empty();
      const exists = this.app.vault.getAbstractFileByPath(path) instanceof import_obsidian.TFile;
      const head = timelineBox.createDiv("calendar-fh-head");
      const back = head.createEl("button", { text: "\u2039 \u8FD4\u56DE" });
      back.className = "calendar-fh-back";
      back.onclick = () => {
        timelineBox.empty();
        input.focus();
      };
      const titleEl = head.createDiv("calendar-fh-title");
      titleEl.textContent = ((_a = entries[0]) == null ? void 0 : _a.title) || path;
      const statusEl = head.createDiv(
        exists ? "calendar-fh-status calendar-fh-status-ok" : "calendar-fh-status calendar-fh-status-gone"
      );
      statusEl.textContent = exists ? "\u73B0\u5B58" : "\u5DF2\u5220\u9664";
      const pathEl = timelineBox.createDiv("calendar-fh-path");
      pathEl.textContent = path;
      const list = timelineBox.createDiv("calendar-fh-list");
      const sorted = [...entries].sort((a, b) => b.updatedAt - a.updatedAt);
      if (sorted.length === 0) {
        list.createDiv("calendar-fh-empty").textContent = "\u6682\u65E0\u6D3B\u52A8\u8BB0\u5F55";
      }
      for (const n of sorted) {
        const d = new Date(n.updatedAt);
        const item = list.createDiv("calendar-fh-item");
        const typeEl = item.createSpan("calendar-fh-item-type calendar-fh-type-" + n.type);
        typeEl.textContent = n.type === "created" ? "\u65B0\u5EFA" : n.type === "deleted" ? "\u5220\u9664" : "\u66F4\u65B0";
        item.createSpan("calendar-fh-item-date").textContent = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
        item.createSpan("calendar-fh-item-time").textContent = `${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
        if (exists) {
          item.onclick = async () => {
    ...
    const renderResults = () => {
      ...
      const hits = [];
      for (const [path, entries] of index) {
        const title = ((_a = entries[0]) == null ? void 0 : _a.title) || path;
        if (title.toLowerCase().includes(kw) || path.toLowerCase().includes(kw)) {
          hits.push({ path, title, ext: ((_b = entries[0]) == null ? void 0 : _b.ext) || "file", count: entries.length });
        }
      }
      hits.sort((a, b) => b.count - a.count);
      if (hits.length === 0) {
        resultBox.createDiv("calendar-fh-empty").textContent = "\u6CA1\u6709\u5339\u914D\u7684\u6587\u4EF6";
        return;
      }
      for (const h of hits.slice(0, 30)) {
        const item = resultBox.createDiv("calendar-fh-hit");
        const extEl = item.createSpan("calendar-note-item-ext");
        extEl.textContent = h.ext;
        item.createSpan("calendar-fh-hit-title").textContent = h.title;
        item.createSpan("calendar-fh-hit-path").textContent = h.path;
        item.createSpan("calendar-fh-hit-count").textContent = `${h.count} \u6761\u8BB0\u5F55`;
        item.onclick = () => showTimeline(h.path, index.get(h.path));
      }
      if (hits.length > 30) {
        resultBox.createDiv("calendar-fh-empty").textContent = `\u5171 ${hits.length} \u4E2A\u5339\u914D\uFF0C\u4EC5\u663E\u793A\u524D 30 \u4E2A`;
      }
    };
...
var CACHE_VERSION = 2;
var CHANGELOG = [
  { version: "1.0.1", date: "2026-09-05", changes: [
    "新增：删除留痕——当天删除的文件在日历留灰色圆点（中性灰 #999，不挤占绿/蓝/琥珀三语义色，深色主题自动适配提亮），统计行显示「删除 X」可点击筛选，文件列表标灰「删除」标签（点击提示已删除）；记录写入 cache.json 重启不丢，全盘重扫自动继承历史删除记录",
    "新增：文件历史搜索——导航栏「⌕」按钮，输入文件名实时匹配标题/路径（含已删除文件，大小写不敏感），点击查看该文件完整生命周期时间线（新建/更新/删除节点 + 日期时间，现存文件可跳转，已删除文件不可跳转）",
    "调整：删除文件不再清除其新建/更新历史（保证时间线完整）；全盘重扫继承「已消失文件」全部历史，同名新建视为新文件重新记录"
  ] },
  { version: "1.0.0", date: "2026-08-09", changes: [
    "\u57FA\u4E8E Note Calendar v1.3.1 (MIT) \u6539\u9020\uFF0C\u66F4\u540D Kumoo Calendar\uFF0C\u4F5C\u8005 KuMoo",
    "\u65B0\u589E\uFF1A\u9EC4\u5386\u9762\u677F\u2014\u2014\u9009\u4E2D\u65E5\u671F\u663E\u793A\u5B9C/\u5FCC\u3001\u5409\u795E/\u51F6\u715E\u3001\u5929\u795E/\u559C\u8D22\u798F\u795E\u65B9\u4F4D\u300112\u65F6\u8FB0\u5409\u51F6\u8868\uFF08lunar-javascript\uFF09",
    "\u65B0\u589E\uFF1A\u5168\u5E93\u6587\u4EF6\u6D3B\u52A8\u7EDF\u8BA1\u2014\u2014\u6240\u6709\u6587\u4EF6\uFF08\u542B\u9644\u4EF6\uFF09\u7684\u521B\u5EFA/\u4FEE\u6539\u65E5\u671F\u6620\u5C04\u5230\u65E5\u5386\uFF0C\u6392\u9664\u9690\u85CF\u76EE\u5F55",
    "\u65B0\u589E\uFF1A\u626B\u63CF\u7ED3\u679C\u78C1\u76D8\u7F13\u5B58\uFF0C\u91CD\u542F\u79D2\u5F00\uFF0C\u6587\u4EF6\u53D8\u5316\u589E\u91CF\u66F4\u65B0",
    "\u65B0\u589E\uFF1A\u70B9\u51FB\u8BBE\u7F6E\u9875\u7248\u672C\u53F7\u53EF\u67E5\u770B\u66F4\u65B0\u65E5\u5FD7"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u6587\u4EF6\u6D3B\u52A8\u8BB0\u5F55\u95EA\u70C1\u2014\u2014\u4E8B\u4EF6\u589E\u91CF\u6539\u4E3A\u5355\u6587\u4EF6 O(1) \u66F4\u65B0 + 600ms \u9632\u6296\u5408\u5E76\uFF0C\u4E0D\u518D\u5168\u5E93\u91CD\u626B",
    "\u4FEE\u590D\uFF1A\u4E0B\u534A\u90E8\u5206\uFF08\u9EC4\u5386+\u6587\u4EF6\u5217\u8868\uFF09\u6574\u4F53\u53EF\u6EDA\u52A8",
    "\u6837\u5F0F\uFF1A\u6309\u7D20\u89C2 SuGuan v1.8.0 \u91CD\u505A\u914D\u8272\uFF08\u53BB\u7D2B\u8272\u7CFB\uFF0C\u7070\u5EA6\u4E3B\u5BFC\uFF0C\u8BED\u4E49\u8272\u9650 3 \u79CD\uFF09"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u518D\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u9EC4\u5386\u9762\u677F\u5076\u53D1\u6574\u4E2A\u4E0D\u663E\u793A\u2014\u2014\u6E32\u67D3\u5168\u7A0B try/catch \u515C\u5E95\uFF0C\u5F02\u5E38\u65F6\u4EC5\u4E22\u5F03\u9762\u677F\uFF0C\u7EDD\u4E0D\u4E2D\u65AD\u6587\u4EF6\u5217\u8868",
    "\u4EA4\u4E92\uFF1A\u9EC4\u5386\u9ED8\u8BA4\u6574\u4F53\u6298\u53E0\uFF08\u53EA\u663E\u793A\u6807\u9898\u884C\uFF0C\u70B9\u51FB\u5C55\u5F00\uFF09\uFF0C\u51CF\u5C11\u4E0B\u534A\u90E8\u5206\u9AD8\u5EA6\u5360\u7528\uFF0C\u7F13\u89E3\u65E0\u6EDA\u52A8\u7A7A\u95F4\u95EE\u9898",
    "\u8FC1\u79FB\uFF1A\u65E7 data.json \u4E2D\u9EC4\u5386\u5C55\u5F00\u72B6\u6001\uFF08\u65E7\u9ED8\u8BA4\u503C\uFF09\u81EA\u52A8\u8FC1\u79FB\u4E3A\u6298\u53E0"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u4E09\u4FEE\u8BA2\uFF09", changes: [
    "\u79FB\u9664\uFF1A\u9EC4\u5386\u9762\u677F\uFF08\u5B9C\u5FCC/\u5409\u795E\u51F6\u715E/\u5929\u795E\u65B9\u4F4D/12\u65F6\u8FB0\uFF09\u2014\u2014\u6309\u7528\u6237\u8981\u6C42\u5F7B\u5E95\u5220\u9664\uFF0C\u754C\u9762\u66F4\u7B80\u6D01",
    "\u65B0\u589E\uFF1A\u6BCF\u65E5\u6587\u4EF6\u6D3B\u52A8\u652F\u6301\u7B5B\u9009\u2014\u2014\u6309\u677F\u5757\uFF08\u8DEF\u5F84\u7B2C\u4E00\u6BB5\u76EE\u5F55\uFF0C\u6839\u76EE\u5F55\u6587\u4EF6\u5F52\u300C\u672A\u5206\u7C7B\u300D\uFF09\u548C\u72B6\u6001\uFF08\u65B0\u5EFA/\u66F4\u65B0\uFF09\u4E0B\u62C9\u7B5B\u9009\uFF0C\u7EDF\u8BA1\u6570\u5B57\u8054\u52A8\u66F4\u65B0"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u56DB\u4FEE\u8BA2\uFF09", changes: [
    "\u4EA4\u4E92\uFF1A\u65B0\u5EFA\u7B14\u8BB0\u6309\u94AE\u667A\u80FD\u5224\u65AD\u2014\u2014\u76EE\u6807\u7B14\u8BB0\uFF08\u65E5\u8BB0/\u5468\u8BB0/\u6708/\u5B63/\u5E74\uFF09\u5DF2\u5B58\u5728\u5219\u76F4\u63A5\u6253\u5F00\uFF0C\u672A\u521B\u5EFA\u5219\u5F39\u51FA\u786E\u8BA4\uFF08\u663E\u793A\u5C06\u521B\u5EFA\u7684\u8DEF\u5F84\uFF09\u540E\u521B\u5EFA\u5E76\u6253\u5F00\uFF0C\u6807\u9898\u81EA\u52A8\u751F\u6210\u4E0D\u518D\u624B\u586B",
    "\u65B0\u589E\uFF1A\u76F8\u5173\u7B14\u8BB0\u6458\u8981\u533A\u2014\u2014\u9009\u4E2D\u65E5\u671F\u5BF9\u5E94\u5DF2\u5B58\u5728\u7684\u65E5\u8BB0/\u5468\u8BB0/\u6708\u5EA6/\u5B63\u5EA6/\u5E74\u5EA6\u7B14\u8BB0\uFF0C\u5728\u6587\u4EF6\u6D3B\u52A8\u5217\u8868\u4E0A\u65B9\u663E\u793A\u7C7B\u578B\u5FBD\u6807+\u6587\u4EF6\u540D+\u5185\u5BB9\u7F29\u7565\uFF0C\u70B9\u51FB\u8DF3\u8F6C\u7B14\u8BB0\u672C\u8EAB\uFF0C\u591A\u6761\u5E76\u5217\u663E\u793A",
    "\u6837\u5F0F\uFF1A\u7B5B\u9009\u4E0B\u62C9\u7D20\u89C2\u5316\u2014\u2014\u53BB\u539F\u751F\u5916\u89C2\u3001\u81EA\u5B9A\u4E49\u7BAD\u5934\uFF08\u9ED8\u8BA4\u7070/\u60AC\u505C\u4EA4\u4E92\u84DD\uFF09\u3001\u6697\u8272\u4E3B\u9898\u9002\u914D\uFF1Bbadge \u7C7B\uFF08\u521B\u5EFA\u6309\u94AE/\u6269\u5C55\u540D\u5FBD\u6807/\u66F4\u65B0\u6807\u7B7E\uFF09\u786C\u7F16\u7801\u8272\u6539\u4E3A\u4E3B\u9898\u53D8\u91CF"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u4E94\u4FEE\u8BA2\uFF09", changes: [
    "\u6837\u5F0F\uFF1A\u7B5B\u9009\u4E0B\u62C9\u60AC\u505C\u53BB\u4EA4\u4E92\u84DD\u2014\u2014\u6309\u7D20\u89C2 6.6 \u7B5B\u9009\u6309\u94AE\u89C4\u8303\uFF0Chover/focus \u8FB9\u6846\u6539 #999\uFF08L4b \u4E13\u7528\u7528\u9014\uFF09\uFF0C\u7BAD\u5934\u60AC\u505C\u6539\u7070\uFF0C\u84DD\u4EC5\u4FDD\u7559\u300C\u4ECA\u5929/\u53EF\u70B9\u51FB\u300D\u951A\u70B9",
    "\u65B0\u589E\uFF1A\u65E5\u671F\u683C\u8BA1\u5212\u70B9\u2014\u2014\u5F53\u5929\u5B58\u5728 \u65E5\u8BB0/\u5468\u8BB0/\u6708\u5EA6/\u5B63\u5EA6/\u5E74\u5EA6 \u4EFB\u4E00\u8BA1\u5212\u7B14\u8BB0\u65F6\uFF0C\u65E5\u671F\u4E0B\u65B9\u4EAE 4px \u7425\u73C0\u70B9\uFF08\u7D20\u89C2\u8BED\u6CD5\u2461\u4FE1\u53F7\u70B9\uFF0C\u540C\u5C4F\u8BED\u4E49\u8272\u6070 3 \u79CD\uFF1A\u7EFF=\u65B0\u5EFA/\u84DD=\u66F4\u65B0/\u7425\u73C0=\u8BA1\u5212\uFF09\uFF1B\u5F52\u5C5E\u903B\u8F91\u4E0E\u6458\u8981\u533A\u4E00\u81F4\uFF08\u5468\u8BB0=\u6574\u5468\u3001\u6708\u5EA6=\u6574\u6708\uFF09\uFF0C\u9009\u4E2D\u65E5\u6DF1\u8272\u5757\u5185\u81EA\u52A8\u63D0\u4EAE"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u516D\u4FEE\u8BA2\uFF09", changes: [
    "\u4EA4\u4E92\uFF1A\u72B6\u6001\u4E0B\u62C9\u6539\u4E3A\u7EDF\u8BA1\u884C\u70B9\u51FB\u7B5B\u9009\u2014\u2014\u79FB\u9664\u300C\u5168\u90E8\u72B6\u6001\u300D\u4E0B\u62C9\uFF1B\u7EDF\u8BA1\u884C\u300C\u65B0\u5EFA X\u300D\u300C\u66F4\u65B0 X\u300D\u53D8\u4E3A\u53EF\u70B9\u51FB\u5F00\u5173\uFF08\u7D20\u89C2 6.6 \u7B5B\u9009\u6309\u94AE\u6FC0\u6D3B\u6001=\u6DF1\u8272\u5757\u767D\u5B57\uFF09\uFF0C\u70B9\u51FB\u53EA\u770B\u8BE5\u7C7B\u3001\u518D\u70B9\u53D6\u6D88\u6062\u590D\u5168\u90E8\uFF1B\u8BA1\u6570\u57FA\u6570\u4E3A\u677F\u5757\u8FC7\u6EE4\u540E\u7C7B\u578B\u8FC7\u6EE4\u524D\uFF0C\u4FDD\u8BC1\u53D6\u6D88\u53EF\u56DE\uFF1B\u677F\u5757\u4E0B\u62C9\u81EA\u9002\u5E94\u5BBD\u5EA6\u4E0D\u518D\u5E73\u5206\u6574\u884C"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u4E03\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u9009\u4E2D\u65E5\u671F\u65E0\u6587\u4EF6\u6D3B\u52A8\u65F6\u519C\u5386\u884C\u4E0E\u4E0B\u65B9\u5185\u5BB9\u91CD\u53E0\u2014\u2014\u519C\u5386\u884C\u539F\u4E3A\u7EDD\u5BF9\u5B9A\u4F4D\uFF0C\u65E0\u6D3B\u52A8\u65E5\u671F\u7EDF\u8BA1\u884C\u4E0D\u6E32\u67D3\u5BFC\u81F4\u884C\u9AD8\u584C\u9677\u4E3A 0\uFF0C\u7EDD\u5BF9\u5B9A\u4F4D\u6587\u672C\u53E0\u5230\u4E0B\u65B9\u5185\u5BB9\u4E0A\uFF1B\u6539\u4E3A flex \u6D41\u5F0F\u5C45\u4E2D\uFF08flex:1+text-align:center\uFF09\uFF0C\u884C\u9AD8\u7531\u5185\u5BB9\u81EA\u7136\u6491\u8D77\uFF0C\u6709/\u65E0\u6D3B\u52A8\u5747\u4E0D\u91CD\u53E0"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u516B\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u521B\u5EFA\u7B14\u8BB0\u6309\u94AE\u7EC4\uFF08+ \u5468 \u6708 \u5B63 \u5E74\uFF09\u538B\u5728\u519C\u5386/\u7EDF\u8BA1\u884C\u4E0A\u2014\u2014\u6309\u94AE\u7EC4\u7EDD\u5BF9\u5B9A\u4F4D\u672A\u8BBE top\uFF0C\u6D4F\u89C8\u5668\u6309\u300C\u5BB9\u5668\u552F\u4E00\u5B50\u5143\u7D20 + justify-content:center\u300D\u8BA1\u7B97\u9759\u6001\u4F4D\u7F6E\uFF0C\u5782\u76F4\u5C45\u4E2D\u843D\u5230\u519C\u5386\u884C\u4E0A\u76D6\u4F4F\u6587\u5B57\uFF1B\u663E\u5F0F top:0 \u5F52\u4F4D\u5230\u6807\u9898\u884C\u5DE6\u4FA7\uFF08\u6807\u9898\u5C45\u4E2D\uFF0C\u5B9E\u6D4B\u6309\u94AE\u53F3\u7F18 1205 < \u6807\u9898\u5DE6\u7F18 1248\uFF0C\u4E0D\u91CD\u53E0\uFF09"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u4E5D\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u65E5\u5386\u683C\u65E5\u671F\u6570\u5B57\u9519\u4F4D\u2014\u2014\u6D3B\u52A8\u5706\u70B9\u4E0E\u8282\u65E5\u6587\u5B57\u539F\u4E3A\u6D41\u5185\u5143\u7D20\uFF0C\u683C\u5B50\u5185\u5BB9\u6574\u4F53\u5782\u76F4\u5C45\u4E2D\u65F6\u5404\u884C\u5185\u5BB9\u9AD8\u5EA6\u4E0D\u540C\uFF0C\u5BFC\u81F4\u6709\u6D3B\u52A8/\u8282\u65E5\u7684\u65E5\u671F\u6570\u5B57\u4E0A\u4E0B\u9519\u843D\uFF1B\u5706\u70B9\u4E0E\u8282\u65E5\u6587\u5B57\u5747\u6539\u4E3A\u7EDD\u5BF9\u5B9A\u4F4D\u8D34\u5E95\uFF08\u8282\u65E5 bottom:5px\u3001\u5706\u70B9 bottom:1px\uFF0C\u540C\u683C\u65F6\u76F8\u5207\u4E0D\u91CD\u53E0\uFF09\uFF0C\u9633\u5386/\u519C\u5386\u6570\u5B57\u5728\u6240\u6709\u683C\u5B50\u4E2D\u4E25\u683C\u5BF9\u9F50",
    "\u6837\u5F0F\uFF1A\u8282\u65E5\u4E0E\u5706\u70B9\u540C\u683C\u65F6\u81EA\u52A8\u5206\u5C42\u8D34\u5E95\uFF1B\u4ECA\u5929\u683C\u6709\u6D3B\u52A8\u70B9\u65F6\u9690\u85CF\u5E95\u90E8\u84DD\u7EBF\uFF08\u84DD\u8272\u6587\u5B57\u4FDD\u7559\u4ECA\u5929\u6807\u8BC6\uFF09\uFF0C\u907F\u514D\u4E0E\u5706\u70B9\u91CD\u53E0",
    '\u4EA4\u4E92\uFF1A\u540C\u65E5\u591A\u8282\u65E5\uFF08\u519C\u5386\u8282\u65E5/\u9633\u5386\u8282\u65E5/\u8282\u6C14\uFF09\u5408\u5E76\u4E3A\u5355\u884C\u663E\u793A\uFF08" \xB7 "\u8FDE\u63A5\uFF09\uFF0C\u4E0D\u518D\u591A\u884C\u5806\u53E0'
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u65E5\u5386\u683C\u5B57\u4F53\u5806\u79EF\u2014\u2014grid \u884C\u9AD8 repeat(6, 1fr) \u5728 auto \u9AD8\u5EA6\u5BB9\u5668\u4E2D\u9000\u5316\u4E3A\u5185\u5BB9\u9AD8\u5EA6\uFF08\u7EA631px\uFF09\uFF0C\u53EA\u591F\u9633\u5386+\u519C\u5386\u4E24\u884C\uFF0C\u8D34\u5E95\u8282\u65E5/\u5706\u70B9\u4E0E\u519C\u5386\u6587\u5B57\u91CD\u53E0\uFF1B\u884C\u9AD8\u6539\u4E3A auto + repeat(6, minmax(58px, 1fr))\uFF08\u7B2C1\u884C=\u5468\u8868\u5934\uFF0C\u540E6\u884C=\u65E5\u5386\u884C\uFF09\uFF0C\u4FDD\u8BC1\u9633\u5386\u519C\u5386\u5C45\u4E2D\u3001\u8282\u65E5(\u8D34\u5E955px)\u3001\u5706\u70B9(\u8D34\u5E951px)\u4E09\u5C42\u7A7A\u95F4\u5145\u8DB3\u4E92\u4E0D\u91CD\u53E0",
    "\u9A8C\u8BC1\uFF1A6 \u884C\u884C\u9AD8\u5747 58px\uFF0C\u6BCF\u884C\u9633\u5386/\u519C\u5386\u6570\u5B57\u5782\u76F4\u4F4D\u7F6E\u5355\u503C\uFF08\u4E25\u683C\u5BF9\u9F50\uFF09\uFF0C\u8282\u65E5+\u5706\u70B9\u540C\u683C\u8FB9\u754C\u76F8\u5207\uFF088/1 \u5EFA\u519B\u8282\u30018/7 \u7ACB\u79CB\uFF09"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u4E00\u4FEE\u8BA2\uFF09", changes: [
    "\u91CD\u6784\uFF1A\u6807\u9898\u533A\u5E03\u5C40\u4ECE\u6DF7\u4E71\u7684 absolute+flex \u6DF7\u5408\u6539\u4E3A\u6E05\u6670\u7684\u4E24\u884C\u7ED3\u6784\u2014\u2014\u7B2C\u4E00\u884C\uFF1A\u6309\u94AE\u7EC4(\u5DE6)+\u6807\u9898(\u4E2D)+\u7EDF\u8BA1\u884C(\u53F3)\u6C34\u5E73\u6392\u5217\uFF1B\u7B2C\u4E8C\u884C\uFF1A\u519C\u5386(\u4E2D)+\u677F\u5757\u4E0B\u62C9(\u53F3)\u6C34\u5E73\u6392\u5217\u3002\u6309\u94AE\u7EC4\u4ECE absolute \u6539\u4E3A inline \u5E03\u5C40\uFF0C\u6D88\u9664\u5B9A\u4F4D\u9677\u9631\uFF0C\u7EC4\u4EF6\u5BF9\u9F50\u6574\u9F50"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u4E8C\u4FEE\u8BA2\uFF09", changes: [
    "\u91CD\u6784\uFF1A\u5E95\u90E8\u9762\u677F\u6539\u4E3A3\u884C\u5E03\u5C40\u2014\u2014\u884C1\uFF1A\u6309\u94AE\u7EC4(\u5DE6)+\u6807\u9898(\u4E2D)+\u7EDF\u8BA1\u884C(\u53F3)\uFF1B\u884C2\uFF1A\u519C\u5386(\u5DE6)\uFF1B\u884C3\uFF1A\u7B5B\u9009\u4E0B\u62C9(\u5DE6)\u3002\u7EDF\u8BA1\u884C\u53F3\u5BF9\u9F50\uFF0C\u519C\u5386\u548C\u7B5B\u9009\u5DE6\u5BF9\u9F50\uFF0C\u5C42\u6B21\u6E05\u6670",
    "\u4F18\u5316\uFF1A\u65E5\u5386\u683C\u884C\u9AD8\u4ECE58px\u964D\u523046px\uFF0C\u9633\u5386+\u519C\u5386(28px)\u5728\u683C\u5B50\u4E2D\u89C6\u89C9\u5C45\u4E2D\uFF08\u4E0A\u4E0B\u5404\u7559~9px\uFF09\uFF0C\u8282\u65E5(\u8D34\u5E955px)\u548C\u5706\u70B9(\u8D34\u5E951px)\u4ECD\u5206\u5C42\u4E0D\u91CD\u53E0"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u4E09\u4FEE\u8BA2\uFF09", changes: [
    "\u91CD\u6784\uFF1A\u5E95\u90E8\u9762\u677F3\u884C\u5E03\u5C40\u7CBE\u8C03\u2014\u2014\u884C1\uFF1A\u6309\u94AE\u7EC4(\u5DE6)+\u6807\u9898(\u7EDD\u5BF9\u5C45\u4E2D)\uFF1B\u884C2\uFF1A\u519C\u5386(\u5C45\u4E2D)\uFF1B\u884C3\uFF1A\u7B5B\u9009\u4E0B\u62C9(\u5DE6)+\u7EDF\u8BA1\u884C(\u53F3)\u3002\u7EDF\u8BA1\u884C\u4ECE\u884C1\u79FB\u81F3\u884C3\u4E0E\u7B5B\u9009\u540C\u884C\uFF0C\u519C\u5386\u4ECE\u884C2\u5DE6\u5BF9\u9F50\u6539\u4E3A\u5C45\u4E2D",
    "\u4FEE\u590D\uFF1A\u6807\u9898\u6C34\u5E73\u5C45\u4E2D\u504F\u79FB\u2014\u2014flex:1 \u6807\u9898\u5143\u7D20\u5728\u6309\u94AE\u7EC4\u53F3\u4FA7\u5360\u5269\u4F59\u7A7A\u95F4\uFF0Ctext-align:center \u4EC5\u5143\u7D20\u5185\u5C45\u4E2D\uFF0C\u5B9E\u9645 centerX \u504F\u79BB\u5BB9\u5668\u4E2D\u5FC3 56px\uFF1B\u6539\u4E3A position:absolute + left:50% + translateX(-50%)\uFF0C\u6807\u9898 centerX \u4E0E\u519C\u5386 centerX \u5B8C\u5168\u5BF9\u9F50\uFF08\u8BEF\u5DEE 0.1px\uFF09"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u56DB\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u65E5\u5386\u683C\u519C\u5386\u6587\u5B57\u4E0E\u8282\u65E5/\u5706\u70B9\u4E09\u5C42\u91CD\u53E0\u2014\u2014\u539F\u884C\u9AD8 46px\uFF08\u5185\u5BB9\u533A 42px\uFF09\u65E0\u6CD5\u5BB9\u7EB3\u9633\u5386+\u519C\u5386\u5C45\u4E2D+\u8282\u65E5\u8D34\u5E95+\u5706\u70B9\u8D34\u5E95\u56DB\u5C42\u5143\u7D20\uFF0C\u5EFA\u519B\u8282/\u7ACB\u79CB/\u4E03\u5915\u8282/\u5904\u6691\u7B49\u65E5\u671F\u519C\u5386\u4E0E\u8282\u65E5\u91CD\u53E0 -1.3px",
    "\u65B9\u6848\uFF1A\u884C\u9AD8 46px\u219258px\uFF08\u5185\u5BB9\u533A 54px\uFF09\uFF0C\u8282\u65E5 bottom:5px\u21926px\u3002\u4E09\u5C42\u5206\u5E03\u5B9E\u6D4B\u2014\u2014\u519C\u5386\u5C45\u4E2D(\u5E95\u224840px) \u2192 \u8282\u65E5(top\u224846px, bottom:6px) \u2192 \u5706\u70B9(top\u224849px, bottom:1px)\uFF0C130 \u683C\u519C\u5386/\u8282\u65E5/\u5706\u70B9\u96F6\u91CD\u53E0"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u4E94\u4FEE\u8BA2\uFF09", changes: [
    "\u4F18\u5316\uFF1A\u65E5\u5386\u683C\u5782\u76F4\u5E03\u5C40\u4ECE\u5C45\u4E2D\u6539\u4E3A\u9760\u4E0A\u2014\u2014justify-content: center \u2192 flex-start\uFF0Cpadding: 2px 1px \u2192 6px 1px 2px\u3002\u9633\u5386\u6570\u5B57\u7D27\u8D34\u9876\u90E8\uFF08offset 0px\uFF09\uFF0C\u6D88\u9664\u4E0A\u65B9 15-20px \u6D6A\u8D39\u7A7A\u95F4\uFF1B\u4E0B\u65B9\u7559\u51FA\u7EA6 30px \u7ED9\u519C\u5386/\u8282\u65E5/\u5706\u70B9\u5206\u5C42\uFF0C\u95F4\u8DDD\u4ECE -1.3px \u6539\u5584\u5230 10px+",
    "\u6548\u679C\uFF1A8\u67087\u65E5\u683C\u5B50\u5B9E\u6D4B\u2014\u2014\u9633\u5386(0px) \u2192 \u519C\u5386(18.5px) \u2192 \u7ACB\u79CB(10.2px) \u2192 \u5706\u70B9(1.0px)\uFF0C\u89C6\u89C9\u5C42\u6B21\u6E05\u6670\uFF0C\u65E0\u91CD\u53E0"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u516D\u4FEE\u8BA2\uFF09", changes: [
    "\u5FAE\u8C03\uFF1A\u8282\u65E5\u5B9A\u4F4D bottom:6px \u2192 10px\uFF0C\u5F80\u4E0A\u632A 4px\u3002\u7ACB\u79CB\u4E0E\u5706\u70B9\u95F4\u8DDD\u4ECE 1.0px \u6539\u5584\u5230 5.0px\uFF0C\u519C\u5386\u4E0E\u7ACB\u79CB\u95F4\u8DDD 6.2px\u3002135 \u683C\u519C\u5386/\u8282\u65E5/\u5706\u70B9\u96F6\u91CD\u53E0",
    "\u5706\u70B9\u8BED\u4E49\uFF1A\u7EFF\u8272=\u65B0\u5EFA\u7B14\u8BB0\uFF0C\u84DD\u8272=\u66F4\u65B0\u7B14\u8BB0\uFF0C\u5C0F\u65B9\u70B9=\u5F53\u65E5\u6709\u65E5\u8BB0/\u5468\u8BB0/\u6708\u8BB0\u7B49\u8BA1\u5212\u7B14\u8BB0"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u4E03\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u624B\u673A\u7248\u300C\u4ECA\u300D\u6309\u94AE\u4E0E\u4E0B\u65B9\u661F\u671F\u8868\u5934\u91CD\u53E0\u2014\u2014\u539F padding:3px 10px + font-size:10.4px \u5BFC\u81F4\u6309\u94AE\u8FC7\u9AD8\uFF1B\u6539\u4E3A padding:2px 8px + font-size:10px + line-height:1.2\uFF0C\u6309\u94AE\u9AD8\u5EA6\u4ECE\u7EA6 20px \u538B\u7F29\u5230\u7EA6 16px\uFF0C\u6D88\u9664\u4E0E\u661F\u671F\u8868\u5934\u7684\u91CD\u53E0"
  ] },
  { version: "1.0.0", date: "2026-08-09\uFF08\u540C\u65E5\u5341\u516B\u4FEE\u8BA2\uFF09", changes: [
    "\u65B0\u589E\uFF1A\u5BFC\u822A\u680F\u624B\u52A8\u626B\u63CF\u6309\u94AE\uFF08\u27F3\uFF09\uFF0C\u4F4D\u4E8E\u300C\u4ECA\u300D\u4E0E\u300C\u203A\u300D\u4E4B\u95F4\u3002\u624B\u673A\u7AEF vault \u540C\u6B65\u540E\u81EA\u52A8\u626B\u63CF\u53EF\u80FD\u672A\u89E6\u53D1\uFF0C\u70B9\u51FB\u6309\u94AE\u53EF\u624B\u52A8\u89E6\u53D1\u5168\u5E93\u626B\u63CF\uFF0C\u626B\u63CF\u671F\u95F4\u663E\u793A\u300C\u6B63\u5728\u626B\u63CF\u6587\u4EF6\u2026\u300D\u63D0\u793A\uFF0C\u5B8C\u6210\u540E\u663E\u793A\u300C\u626B\u63CF\u5B8C\u6210\u300D"
  ] },
  { version: "1.0.0", date: "2026-08-10\uFF08\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u624B\u673A\u7AEF\u9ED8\u8BA4\u4E0D\u52A0\u8F7D\u6587\u4EF6\u6D3B\u52A8\u7F13\u5B58\u2014\u2014\u626B\u63CF\u7ED3\u679C\u4E0E\u89C6\u56FE\u89E3\u8026\uFF1A\u7ED3\u679C\u5148\u6682\u5B58\u63D2\u4EF6\u7EA7 pendingNoteCache\uFF0Cview \u4F55\u65F6\u521B\u5EFA/\u5C31\u7EEA\u90FD\u80FD\u5728\u6E32\u67D3\u65F6\u81EA\u52A8\u53D6\u7528\uFF0C\u4E0D\u518D\u4F9D\u8D56\u300C1\u79D2\u540E view \u5DF2\u5C31\u7EEA\u300D\u7684\u65F6\u5E8F\u5047\u8BBE\uFF08\u624B\u673A\u7AEF setViewState \u5F02\u6B65\u521B\u5EFA\u6162\uFF09",
    "\u4FEE\u590D\uFF1A\u624B\u673A\u7AEF\u589E\u91CF\u66F4\u65B0\u5728 view \u672A\u5C31\u7EEA\u65F6\u4E0D\u518D\u4E22\u5931\u2014\u2014\u4E8B\u4EF6\u589E\u91CF\u540C\u6837\u4F5C\u7528\u4E8E\u63D2\u4EF6\u7EA7\u6682\u5B58\u7F13\u5B58\uFF0Cview \u521B\u5EFA\u540E\u4E00\u5E76\u751F\u6548",
    "\u4F18\u5316\uFF1A\u5168\u91CF\u626B\u63CF\u6539\u7528 vault \u7D22\u5F15\u81EA\u5E26 stat\uFF08\u540C\u6B65\u96F6 I/O\uFF09\uFF0Cadapter.stat \u4EC5\u515C\u5E95\uFF0C\u624B\u673A\u6162\u8BBE\u5907\u626B\u63CF\u4E0D\u518D\u957F\u65F6\u95F4\u65E0\u54CD\u5E94",
    "\u9632\u62A4\uFF1A\u7A7A\u626B\u63CF\u7ED3\u679C\u4E0D\u5199\u56DE cache.json\uFF0C\u4E14\u7A7A\u7F13\u5B58\u4E0D\u89C6\u4E3A\u6709\u6548\u547D\u4E2D\u2014\u2014\u624B\u673A\u7AEF\u626B\u63CF\u5931\u8D25\u65F6\u4E0D\u518D\u628A\u78C1\u76D8\u7F13\u5B58\u6C61\u67D3\u4E3A\u7A7A\uFF0C\u5373\u4F7F\u5386\u53F2\u7F13\u5B58\u5DF2\u88AB\u6C61\u67D3\uFF0C\u6253\u5F00\u5373\u81EA\u52A8\u5168\u91CF\u91CD\u5EFA\uFF0C\u4E0D\u518D\u6C38\u4E45\u7A7A\u767D"
  ] },
  { version: "1.0.0", date: "2026-08-10\uFF08\u540C\u65E5\u518D\u4FEE\u8BA2\uFF09", changes: [
    "\u4FEE\u590D\uFF1A\u79FB\u52A8\u7AEF\u81EA\u52A8\u91CD\u626B\u5168\u76D8\u8986\u76D6 PC \u7F13\u5B58\u6570\u636E\uFF08\u5B9E\u6D4B 8/7 \u65B0\u5EFA 22\u219215\u3001\u66F4\u65B0 3\u21920\uFF09\u2014\u2014\u540C\u6B65\u8FC7\u7A0B\u4F1A\u6539\u5199\u6587\u4EF6\u521B\u5EFA/\u4FEE\u6539\u65F6\u95F4\uFF0C\u79FB\u52A8\u7AEF\u5168\u76D8\u626B\u63CF\u5FC5\u7136\u5931\u771F\uFF1B\u6539\u4E3A\u79FB\u52A8\u7AEF\u7F13\u5B58\u53EA\u8BFB\uFF1A\u5B58\u5728\u6709\u6548\u7F13\u5B58\u5373\u76F4\u63A5\u52A0\u8F7D PC \u6570\u636E\uFF0C\u7EDD\u4E0D\u81EA\u52A8\u91CD\u626B",
    "\u4FEE\u590D\uFF1A\u79FB\u52A8\u7AEF\u4E0D\u518D\u5199\u56DE cache.json\u2014\u2014\u6B64\u524D\u79FB\u52A8\u7AEF\u6BCF\u6B21\u91CD\u626B\u90FD\u4F1A\u628A\u5931\u771F\u7ED3\u679C\u5199\u56DE\uFF0C\u6C61\u67D3\u78C1\u76D8\u7F13\u5B58\u5E76\u968F\u540C\u6B65\u6269\u6563\u56DE PC\uFF1B\u73B0\u79FB\u52A8\u7AEF\u4EC5 PC \u5199\u7F13\u5B58",
    "\u7F13\u5B58\u683C\u5F0F\u5347\u7EA7 v2\uFF1A\u4F7F\u79FB\u52A8\u7AEF\u5DF2\u6C61\u67D3\u7684\u65E7\u7F13\u5B58\u5931\u6548\uFF0CPC \u7AEF\u91CD\u542F\u540E\u81EA\u52A8\u91CD\u5EFA\u6B63\u786E\u7F13\u5B58",
    "\u4EA4\u4E92\uFF1A\u27F3 \u6309\u94AE\u6539\u4E3A\u5F3A\u5236\u626B\u63CF\uFF08\u7ED5\u8FC7\u7F13\u5B58\u76F4\u63A5\u5168\u76D8\u626B\u63CF\uFF09\u2014\u2014\u79FB\u52A8\u7AEF\u7F13\u5B58\u6709\u6548\u65F6\u624B\u52A8\u6309\u94AE\u4ECD\u53EF\u4E3B\u52A8\u5237\u65B0\uFF0C\u7ED3\u679C\u4EC5\u672C\u6B21\u4F1A\u8BDD\u751F\u6548"
  ] },
  { version: "1.0.0", date: "2026-08-10\uFF08\u540C\u65E5\u4E09\u4FEE\u8BA2\uFF09", changes: [
    "\u65B0\u589E\uFF1A\u5220\u9664\u7559\u75D5\u2014\u2014\u5F53\u5929\u5220\u9664\u7684\u6587\u4EF6\u5728\u65E5\u5386\u4E0A\u7559\u7070\u8272\u5706\u70B9\uFF0C\u6587\u4EF6\u5217\u8868\u663E\u793A\u7070\u8272\u300C\u5220\u9664\u300D\u6807\u7B7E\uFF1B\u7EDF\u8BA1\u884C\u300C\u5220\u9664 X\u300D\u53EF\u70B9\u51FB\u7B5B\u9009\uFF08\u4EC5\u5F53\u65E5\u6709\u5220\u9664\u8BB0\u5F55\u65F6\u663E\u793A\uFF09\uFF0C\u70B9\u51FB\u5DF2\u5220\u9664\u6587\u4EF6\u63D0\u793A\u300C\u8BE5\u6587\u4EF6\u5DF2\u5220\u9664\u300D",
    "\u7EC6\u8282\uFF1A\u5220\u9664\u8BB0\u5F55\u4F1A\u6E05\u9664\u8BE5\u6587\u4EF6\u65E7\u7684\u65B0\u5EFA/\u66F4\u65B0\u6761\u76EE\uFF08\u6587\u4EF6\u5DF2\u4E0D\u5B58\u5728\uFF0C\u53EA\u5728\u5220\u9664\u65E5\u7559\u75D5\uFF09\uFF1B\u5168\u76D8\u91CD\u626B\u65F6\u81EA\u52A8\u4ECE\u65E7\u7F13\u5B58\u7EE7\u627F\u5220\u9664\u8BB0\u5F55\uFF0C\u6587\u4EF6\u540C\u540D\u6062\u590D\u5219\u4F5C\u5E9F",
    "\u914D\u8272\uFF1A\u5220\u9664\u4FE1\u53F7\u7528\u4E2D\u6027\u7070\uFF08#999\uFF09\u2014\u2014\u4E0D\u6324\u5360\u7EFF(\u65B0\u5EFA)/\u84DD(\u66F4\u65B0)/\u7425\u73C0(\u8BA1\u5212)\u4E09\u8BED\u4E49\u8272\uFF0C\u6DF1\u8272\u4E3B\u9898\u81EA\u52A8\u9002\u914D"
  ] },
  { version: "1.0.0", date: "2026-08-10\uFF08\u540C\u65E5\u56DB\u4FEE\u8BA2\uFF09", changes: [
    "\u65B0\u589E\uFF1A\u6587\u4EF6\u5386\u53F2\u641C\u7D22\u2014\u2014\u5BFC\u822A\u680F\u300C\u2315\u300D\u6309\u94AE\u6253\u5F00\u5F39\u7A97\uFF0C\u8F93\u5165\u6587\u4EF6\u540D\u5B9E\u65F6\u5339\u914D\uFF08\u542B\u5DF2\u5220\u9664\u6587\u4EF6\uFF09\uFF0C\u70B9\u51FB\u6587\u4EF6\u67E5\u770B\u5176\u65B0\u5EFA/\u66F4\u65B0/\u5220\u9664\u7684\u5B8C\u6574\u65F6\u95F4\u7EBF\uFF08\u65E5\u671F+\u65F6\u95F4+\u7C7B\u578B\u5FBD\u6807\uFF09\uFF0C\u70B9\u51FB\u65F6\u95F4\u7EBF\u6761\u76EE\u8DF3\u8F6C\u6587\u4EF6\u672C\u4F53",
    "\u8C03\u6574\uFF1A\u5220\u9664\u6587\u4EF6\u4E0D\u518D\u6E05\u9664\u5176\u5386\u53F2\u7684\u65B0\u5EFA/\u66F4\u65B0\u8BB0\u5F55\u2014\u2014\u65F6\u95F4\u7EBF\u9700\u8981\u5B8C\u6574\u751F\u547D\u5468\u671F\uFF1B\u5168\u76D8\u91CD\u626B\u65F6\u300C\u5DF2\u6D88\u5931\u6587\u4EF6\u300D\u7684\u5168\u90E8\u5386\u53F2\u6761\u76EE\u4ECE\u65E7\u7F13\u5B58\u7EE7\u627F\uFF0C\u540C\u540D\u6062\u590D\u5219\u89C6\u4E3A\u65B0\u6587\u4EF6",
    "\u65F6\u95F4\u7EBF\u5934\u90E8\u663E\u793A\u6587\u4EF6\u5F53\u524D\u72B6\u6001\uFF08\u73B0\u5B58/\u5DF2\u5220\u9664\uFF09\uFF0C\u5DF2\u5220\u9664\u6587\u4EF6\u4E0D\u53EF\u8DF3\u8F6C\uFF1B\u641C\u7D22\u7ED3\u679C\u6309\u8BB0\u5F55\u6570\u6392\u5E8F\uFF0C\u6700\u591A\u663E\u793A 30 \u6761"
  ] }
];
function isExcludedPath(path) {
  return path.split("/").some((seg) => seg.startsWith("."));
}
var NoteCalendarPlugin = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
    this.cacheSaveTimer = null;
    /** 事件增量队列（600ms 防抖合并，避免同步风暴逐条处理） */
    this.pendingIncrements = [];
    this.incrementTimer = null;
    /**
     * 扫描结果插件级暂存（v1.0.0 十九修订）：
     * 手机端 setViewState 异步创建 view 慢，定时扫描可能早于 view 就绪而静默跳过；
     * 结果先存此处，view 创建后渲染时自动取用，扫描与视图彻底解耦。
     */
    this.pendingNoteCache = null;
  }
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on("create", (file) => {
        if (isExcludedPath(file.path))
          return;
        this.queueIncrement(file.path, "create");
      })
    );
    this.registerEvent(
      this.app.vault.on("modify", (file) => {
        if (isExcludedPath(file.path))
          return;
        this.queueIncrement(file.path, "modify");
      })
    );
    this.registerEvent(
      this.app.vault.on("rename", (file, oldPath) => {
        if (isExcludedPath(file.path))
          return;
        this.queueIncrement(file.path, "rename", oldPath);
      })
    );
    this.registerEvent(
      this.app.vault.on("delete", (file) => {
        if (isExcludedPath(file.path))
          return;
        this.queueIncrement(file.path, "delete");
      })
    );
  }
  async loadSettings() {
    ...
    if (data) {
      if (data.themeColor === "#5d4ed8")
        data.themeColor = DEFAULT_SETTINGS.themeColor;
      if (data.weekendColor === "#e57373")
        data.weekendColor = DEFAULT_SETTINGS.weekendColor;
    }
  ...
  async updateSettings(newSettings) {
    ...
    leaves.forEach((leaf) => {
      ...
      if (view.model) {
        const m = view.model;
        m.startOfWeek = this.settings.startOfWeek;
        m.weekendColor = this.settings.weekendColor;
        m.themeColor = this.settings.themeColor;
        m.followAccentColor = this.settings.followAccentColor;
        m.showLunarDate = this.settings.showLunarDate;
        m.showSolarFestivals = this.settings.showSolarFestivals;
        m.showLunarFestivals = this.settings.showLunarFestivals;
        m.showJieQi = this.settings.showJieQi;
        m.showHolidayMarker = this.settings.showHolidayMarker;
        m.noteFolderPath = this.settings.noteFolderPath;
        m.fontFamily = this.settings.fontFamily;
        m.fontSize = this.settings.fontSize;
        m.themeMode = this.settings.themeMode;
        m.dailyTitleFormat = this.settings.dailyTitleFormat;
        m.dailyFolderPath = this.settings.dailyFolderPath;
        m.weeklyTitleFormat = this.settings.weeklyTitleFormat;
        m.weeklyFolderPath = this.settings.weeklyFolderPath;
        m.quarterlyTitleFormat = this.settings.quarterlyTitleFormat;
        m.quarterlyFolderPath = this.settings.quarterlyFolderPath;
        m.yearlyTitleFormat = this.settings.yearlyTitleFormat;
        m.yearlyFolderPath = this.settings.yearlyFolderPath;
        m.monthlyTitleFormat = this.settings.monthlyTitleFormat;
        m.monthlyFolderPath = this.settings.monthlyFolderPath;
        m.showQuarterly = this.settings.showQuarterly;
        m.quarterlyMode = this.settings.quarterlyMode;
        m.quarterStartMonth = this.settings.quarterStartMonth;
        m.quarterlyCustomNames = this.settings.quarterlyCustomNames;
        view.render();
      }
    });
  }
  /**
   * 获取本次扫描的文件集合（排除隐藏路径 + 可选限定扫描目录）
   */
  getScannableFiles() {
    let files = this.app.vault.getFiles().filter((f) => !isExcludedPath(f.path));
    if (this.settings.noteFolderPath) {
      const folder = this.settings.noteFolderPath.endsWith("/") ? this.settings.noteFolderPath : this.settings.noteFolderPath + "/";
  ...
  /**
   * 全库扫描：所有文件（含附件）按创建/修改日期建立活动索引
   * 优先命中磁盘缓存（文件路径集合一致时直接加载，秒开不重扫）
   * 集合不一致时：先用旧缓存渲染（避免空白闪烁），再后台全量刷新
   *
   * 移动端策略（v1.0.0 二十修订）：
   * - 缓存存在且有效 → 直接加载，绝不自动重扫全盘。
   *   同步过程会改写文件 ctime/mtime（大量文件归到同步当天、更新标记丢失），
   *   移动端全盘扫描必然失真（如 8/7 新建 22→15、更新 3→0），PC 同步来的缓存才是准的。
   * - 缓存不存在/无效（版本不符、为空）→ 全量扫描兜底（仅首次/升级，不写回）。
   * - force=true（手动 ⟳）→ 强制全量扫描，本次会话生效，不写回。
   */
  async scanNotes(force = false) {
    const files = this.getScannableFiles();
    let cached = null;
    try {
      cached = await this.loadScanCache();
    } catch (e) {
      console.warn("[KumooCalendar] \u8BFB\u53D6\u626B\u63CF\u7F13\u5B58\u5931\u8D25\uFF0C\u6267\u884C\u5168\u91CF\u626B\u63CF:", e);
    }
    const cacheValid = cached && cached.version === CACHE_VERSION && cached.noteCache && Object.keys(cached.noteCache).length > 0;
    if (cacheValid && !force && cached) {
      if (import_obsidian2.Platform.isMobile) {
        this.applyNoteCache(cached.noteCache);
        return;
      }
      if (this.sameFileSet(files, cached.filePaths)) {
        this.applyNoteCache(cached.noteCache);
        return;
      }
      this.applyNoteCache(cached.noteCache);
    }
    const noteCache = await this.scanAllFiles(files);
    this.mergeVanishedRecords(noteCache, cached ? cached.noteCache : null);
    this.mergeVanishedRecords(noteCache, this.pendingNoteCache);
    this.applyNoteCache(noteCache);
    await this.saveCacheFromModel();
  }
  /**
   * 删除留痕（v1.0.0 二十一修订）：在删除当天记录一条「删除」。
   * v1.0.0 同日四修订：不再清除该文件旧的新建/更新条目——文件历史时间线
   * 需要完整生命周期（新建→更新→删除），旧记录保留在原日期。
   */
  recordDeletedFile(cache, filePath) {
    if (!this.isInScanScope(filePath))
      return;
    const name = filePath.split("/").pop() || filePath;
    const dot = name.lastIndexOf(".");
    const ext = dot > 0 ? name.slice(dot + 1) : "file";
    const title = ext === "md" ? name.slice(0, -3) : name;
    const entry = { path: filePath, title, type: "deleted", updatedAt: Date.now(), ext };
    const dateStr = this.fmtDate(Date.now());
    if (!cache[dateStr])
      cache[dateStr] = [];
    cache[dateStr].push(entry);
  }
  /**
   * 合并「已消失文件」（被删除/移出扫描范围）的全部历史条目到新扫描结果：
   * 全盘扫描只能看到现存文件，已消失文件的记录必须从旧缓存继承；
   * 文件若重新出现（同名新建/恢复），视为新文件，旧记录作废丢弃。
   */
  mergeVanishedRecords(noteCache, prev) {
    if (!prev)
      return;
    const existing = new Set(this.app.vault.getFiles().map((f) => f.path));
    for (const dateStr in prev) {
      for (const n of prev[dateStr]) {
        if (existing.has(n.path))
          continue;
        if (!noteCache[dateStr])
          noteCache[dateStr] = [];
        if (!noteCache[dateStr].some((m) => m.type === n.type && m.path === n.path && m.updatedAt === n.updatedAt)) {
          noteCache[dateStr].push(n);
        }
      }
    }
  }
  /**
   * 应用扫描结果：先暂存插件级（view 未就绪也不丢），view 存在则立即赋值渲染。
   * view 的 render() 另有兜底取用 pendingNoteCache，保证手机端慢启动也能显示。
   * 空结果不应用：无价值且避免覆盖已有数据造成闪烁。
   */
  applyNoteCache(noteCache) {
    if (!noteCache || Object.keys(noteCache).length === 0)
      return;
    this.pendingNoteCache = noteCache;
    const view = this.getCalendarView();
    if (!view)
      return;
    view.model.noteCache = noteCache;
    view.render();
  }
  /**
   * 全库批量扫描（40 并发批次），返回新索引
   * 手机端修复（v1.0.0 十九修订）：优先用 vault 索引自带的 file.stat（同步零 I/O，
   * 慢设备扫描不再超时），缺失时回退 adapter.stat；不依赖 view/model。
   */
  async scanAllFiles(files) {
    ...
    for (let i = 0; i < files.length; i += SCAN_BATCH) {
      ...
      await Promise.all(batch.map(async (file) => {
        var _a;
        try {
          let stat = (_a = file.stat) != null ? _a : null;
          if (!stat || !stat.ctime) {
            stat = await this.app.vault.adapter.stat(file.path);
          }
          if (!stat)
            return;
          const createdDateStr = fmt(stat.ctime);
          const modifiedDateStr = fmt(stat.mtime);
          const title = file.extension === "md" ? file.basename : file.name;
          const ext = file.extension || "file";
          const entry = { path: file.path, title, type: "created", updatedAt: stat.mtime, ext };
          if (!noteCache[createdDateStr])
            noteCache[createdDateStr] = [];
          noteCache[createdDateStr].push(entry);
          if (modifiedDateStr !== createdDateStr) {
            if (!noteCache[modifiedDateStr])
              noteCache[modifiedDateStr] = [];
            noteCache[modifiedDateStr].push({ ...entry, type: "updated" });
          }
        } catch (e) {
          console.error(`[KumooCalendar] \u5904\u7406\u6587\u4EF6 ${file.path} \u51FA\u9519:`, e);
        }
      }));
    }
    return noteCache;
  }
  /** 缓存文件路径：插件目录/cache.json */
  get cachePath() {
  ...
  /**
   * 从当前 model 状态构建并写入缓存
   * 移动端只读（v1.0.0 二十修订）：磁盘缓存一律以 PC 生成的数据为准——
   * 移动端扫描结果因同步失真，写回会把错误数据同步回 PC 污染两端。
   */
  async saveCacheFromModel() {
    if (import_obsidian2.Platform.isMobile)
      return;
    const leaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_CALENDAR);
    if (leaves.length === 0)
    ...
    if (!view || !view.model)
      return;
    if (!view.model.noteCache || Object.keys(view.model.noteCache).length === 0)
      return;
    const cache = {
      version: CACHE_VERSION,
      scanTime: Date.now(),
      filePaths: this.getScannableFiles().map((f) => f.path).sort(),
      noteCache: view.model.noteCache
    };
    try {
      await this.app.vault.adapter.write(this.cachePath, JSON.stringify(cache));
    } catch (e) {
      console.warn("[KumooCalendar] \u5199\u5165\u626B\u63CF\u7F13\u5B58\u5931\u8D25:", e);
    }
  }
  /** 缓存保存防抖（文件事件频繁触发时合并写入） */
  scheduleCacheSave() {
    if (this.cacheSaveTimer)
      window.clearTimeout(this.cacheSaveTimer);
    this.cacheSaveTimer = window.setTimeout(() => {
      this.cacheSaveTimer = null;
      this.saveCacheFromModel();
    }, 3e3);
  }
  /** 判断文件集合与缓存记录是否一致（数量 + 路径集合） */
  sameFileSet(files, cachedPaths) {
    if (files.length !== cachedPaths.length)
      return false;
    const set = new Set(cachedPaths);
    return files.every((f) => set.has(f.path));
  }
  /**
   * 事件增量入队（600ms 防抖合并）
   * 观察者公开入口：view.ts 创建笔记后也可直接调用
   */
  queueIncrement(path, kind, oldPath) {
    this.pendingIncrements.push({ path, kind, oldPath });
  ...
  /**
   * 批量执行增量队列：单文件 O(1) 更新，不触发全库扫描，无闪烁
   * 手机端修复（v1.0.0 十九修订）：view 未就绪时增量同样作用于插件级暂存缓存，不丢失
   */
  async flushIncrements() {
    var _a;
    this.incrementTimer = null;
    const items = this.pendingIncrements;
    this.pendingIncrements = [];
    if (items.length === 0)
      return;
    const view = this.getCalendarView();
    const cache = view ? view.model.noteCache : (_a = this.pendingNoteCache) != null ? _a : {};
    for (const item of items) {
      try {
        if (item.kind === "delete") {
          this.recordDeletedFile(cache, item.path);
        } else if (item.kind === "rename") {
          this.removePathFromCache(cache, item.oldPath || item.path);
          await this.incrementalUpdateFile(cache, item.path);
        } else {
          await this.incrementalUpdateFile(cache, item.path);
        }
      } catch (e) {
        console.error(`[KumooCalendar] \u589E\u91CF\u66F4\u65B0 ${item.path} \u5931\u8D25:`, e);
      }
    }
    if (!view) {
      this.pendingNoteCache = cache;
      return;
    }
    this.pendingNoteCache = view.model.noteCache;
    view.render();
    this.scheduleCacheSave();
  }
  /** 从缓存对象中移除某路径的全部条目（所有日期） */
  removePathFromCache(cache, filePath) {
    for (const dateStr in cache) {
      const arr = cache[dateStr];
      if (arr.some((n) => n.path === filePath)) {
        cache[dateStr] = arr.filter((n) => n.path !== filePath);
        if (cache[dateStr].length === 0)
          delete cache[dateStr];
      }
    }
  }
  /** 该文件是否在扫描范围内（隐藏路径 + 限定目录） */
  isInScanScope(filePath) {
   ...
   * 单文件增量更新（O(1)，不扫描全库）：
   * 1. 从缓存移除该路径所有旧条目
   * 2. stat 该文件，按创建/修改日期重新插入
   * 全程无"缓存被清空"的中间态，日历标记不会闪烁
   */
  async incrementalUpdateFile(cache, filePath) {
    var _a;
    this.removePathFromCache(cache, filePath);
    if (!this.isInScanScope(filePath))
      return;
    const file = this.app.vault.getAbstractFileByPath(filePath);
    if (!file || !(file instanceof import_obsidian2.TFile))
      return;
    const stat = (_a = file.stat) != null ? _a : await this.app.vault.adapter.stat(filePath);
    if (!stat)
      return;
    const createdStr = this.fmtDate(stat.ctime);
    const modifiedStr = this.fmtDate(stat.mtime);
    const title = file.extension === "md" ? file.basename : file.name;
    const ext = file.extension || "file";
    const entry = { path: filePath, title, type: "created", updatedAt: stat.mtime, ext };
    if (!cache[createdStr])
      cache[createdStr] = [];
    cache[createdStr].push(entry);
    if (modifiedStr !== createdStr) {
      if (!cache[modifiedStr])
        cache[modifiedStr] = [];
      cache[modifiedStr].push({ ...entry, type: "updated" });
    }
  }
  /** 毫秒时间戳 → YYYY-MM-DD（本地时区，与 model.formatDate 一致） */
  fmtDate(ts) {
    const d = new Date(ts);
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
  }
  ...
  initLeaf() {
    ...
    if (this.app.workspace.getLeavesOfType(VIEW_TYPE_CALENDAR).length > 0)
      return;
    (_a = this.app.workspace.getRightLeaf(false)) == null ? void 0 : _a.setViewState({ type: VIEW_TYPE_CALENDAR });
  }
...
var CalendarSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Kumoo Calendar \u8BBE\u7F6E" });
    const aboutSection = this.createSection("\u5173\u4E8E");
    new import_obsidian2.Setting(aboutSection).setName("Kumoo Calendar").setDesc("\u4F5C\u8005\uFF1AKuMoo \xB7 \u57FA\u4E8E Note Calendar v1.3.1 (MIT) \u6539\u9020").addButton(
      (button) => button.setButtonText(`v${this.plugin.manifest.version} \xB7 \u67E5\u770B\u66F4\u65B0\u65E5\u5FD7`).onClick(() => {
        new ChangelogModal(this.app, this.plugin.manifest.version).open();
      })
    );
    const appearanceSection = this.createSection("\u5916\u89C2");
    new import_obsidian2.Setting(appearanceSection).setName("\u4E3B\u9898\u6A21\u5F0F").setDesc("\u9009\u62E9\u65E5\u5386\u80CC\u666F\u4E3B\u9898\u6A21\u5F0F\u3002\u8DDF\u968FObsidian\u5C06\u81EA\u52A8\u9002\u914D\u6DF1\u8272/\u6D45\u8272\u4E3B\u9898").addDropdown(
      (dropdown) => dropdown.addOption("auto", "\u8DDF\u968FObsidian").addOption("dark", "\u6DF1\u8272").addOption("light", "\u6D45\u8272").setValue(this.plugin.settings.themeMode || "auto").onChange(async (value) => {
        await this.plugin.updateSettings({ themeMode: value });
      })
    );
    new import_obsidian2.Setting(appearanceSection).setName("\u4E00\u5468\u8D77\u59CB\u65E5").setDesc("\u9009\u62E9\u65E5\u5386\u4E00\u5468\u7684\u7B2C\u4E00\u5929\u662F\u5468\u65E5\u8FD8\u662F\u5468\u4E00").addDropdown(
      (dropdown) => dropdown.addOption("0", "\u5468\u65E5").addOption("1", "\u5468\u4E00").setValue(String(this.plugin.settings.startOfWeek)).onChange(async (value) => {
        await this.plugin.updateSettings({ startOfWeek: parseInt(value) });
      })
    );
    new import_obsidian2.Setting(appearanceSection).setName("\u5468\u672B\u989C\u8272").setDesc("\u5468\u516D\u5468\u65E5\u7684\u663E\u793A\u989C\u8272").addColorPicker(
      (colorPicker) => colorPicker.setValue(this.plugin.settings.weekendColor).onChange(async (value) => {
        await this.plugin.updateSettings({ weekendColor: value });
      })
    );
    new import_obsidian2.Setting(appearanceSection).setName("\u8DDF\u968F Obsidian \u5F3A\u8C03\u8272").setDesc("\u5F00\u542F\u540E\u4E3B\u9898\u8272\u5B9E\u65F6\u8DDF\u968F Obsidian \u7684\u5F3A\u8C03\u8272\uFF08\u8BBE\u7F6E \u2192 \u5916\u89C2 \u2192 \u5F3A\u8C03\u8272\uFF09\uFF0C\u5E76\u9690\u85CF\u4E0B\u65B9\u7684\u4E3B\u9898\u989C\u8272\u914D\u7F6E\u9879").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.followAccentColor).onChange(async (value) => {
        await this.plugin.updateSettings({ followAccentColor: value });
      })
    );
    if (!this.plugin.settings.followAccentColor) {
      new import_obsidian2.Setting(appearanceSection).setName("\u4E3B\u9898\u989C\u8272").setDesc("\u65E5\u5386\u7684\u4E3B\u9898\u989C\u8272").addColorPicker(
        (colorPicker) => colorPicker.setValue(this.plugin.settings.themeColor).onChange(async (value) => {
          await this.plugin.updateSettings({ themeColor: value });
        })
      );
    }
    new import_obsidian2.Setting(appearanceSection).setName("\u5B57\u4F53").setDesc("\u9009\u62E9\u65E5\u5386\u4F7F\u7528\u7684\u5B57\u4F53").addDropdown(
      (dropdown) => dropdown.addOption("default", "\u9ED8\u8BA4").addOption("microsoft-yahei", "\u5FAE\u8F6F\u96C5\u9ED1").addOption("simsun", "\u5B8B\u4F53").addOption("simhei", "\u9ED1\u4F53").addOption("arial", "Arial").addOption("helvetica", "Helvetica").addOption("verdana", "Verdana").addOption("tahoma", "Tahoma").addOption("segoe-ui", "Segoe UI").setValue(this.plugin.settings.fontFamily).onChange(async (value) => {
        await this.plugin.updateSettings({ fontFamily: value });
      })
    );
    new import_obsidian2.Setting(appearanceSection).setName("\u5B57\u53F7").setDesc("\u8BBE\u7F6E\u65E5\u5386\u6587\u5B57\u5927\u5C0F\uFF0810-20px\uFF09").addSlider(
      (slider) => slider.setLimits(10, 20, 1).setValue(this.plugin.settings.fontSize).setDynamicTooltip().onChange(async (value) => {
        await this.plugin.updateSettings({ fontSize: value });
      })
    );
    const almanacSection = this.createSection("\u6587\u4EF6\u6D3B\u52A8\u7EDF\u8BA1\uFF08Plus\uFF09");
    new import_obsidian2.Setting(almanacSection).setName("\u7EDF\u8BA1\u8303\u56F4").setDesc("Plus: \u7EDF\u8BA1\u6574\u4E2A vault \u6240\u6709\u6587\u4EF6\uFF08\u542B\u56FE\u7247/\u9644\u4EF6\uFF09\u7684\u521B\u5EFA\u4E0E\u4FEE\u6539\u65E5\u671F\uFF1B\u9690\u85CF\u76EE\u5F55(.obsidian/.trash/.git\u7B49)\u81EA\u52A8\u6392\u9664").setDisabled(true);
    ...
    const quarterSection = this.createSection("\u5B63\u5EA6\u663E\u793A");
    new import_obsidian2.Setting(quarterSection).setName("\u663E\u793A\u5B63\u5EA6").setDesc("\u5728\u65E5\u5386\u6807\u9898\u4E2D\u663E\u793A\u5F53\u524D\u5B63\u5EA6").addToggle((toggle) => {
    ...
    new import_obsidian2.Setting(quarterSection).setName("\u5B63\u5EA6\u663E\u793A\u6A21\u5F0F").setDesc("\u9009\u62E9\u5B63\u5EA6\u7684\u663E\u793A\u65B9\u5F0F").addDropdown((dropdown) => {
      const currentMode = this.plugin.settings.quarterlyMode;
      dropdown.addOption("number", "\u6570\u5B57\uFF081\u5B63\u5EA6\u30012\u5B63\u5EA6\u2026\uFF09").addOption("season", "\u6625\u590F\u79CB\u51AC\uFF08\u6625\u5B63\u3001\u590F\u5B63\u2026\uFF09").addOption("custom", "\u81EA\u5B9A\u4E49\u547D\u540D");
    ...
    if (this.plugin.settings.quarterlyMode === "custom") {
      new import_obsidian2.Setting(quarterSection).setName("\u81EA\u5B9A\u4E49\u5B63\u5EA6\u540D\u79F0").setDesc("\u7528\u9017\u53F7\u5206\u9694\u56DB\u4E2A\u5B63\u5EA6\u7684\u540D\u79F0\uFF0C\u9700\u6070\u597D4\u4E2A\u3002\u4F8B\u5982\uFF1AQ1,Q2,Q3,Q4").addText(
        (text) => text.setPlaceholder("\u6625\u5B63,\u590F\u5B63,\u79CB\u5B63,\u51AC\u5B63").setValue(this.plugin.settings.quarterlyCustomNames).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(quarterSection).setName("\u9996\u5B63\u8D77\u59CB\u6708\u4EFD").setDesc("\u8BBE\u7F6E\u7B2C\u4E00\u4E2A\u5B63\u5EA6\u4ECE\u54EA\u4E2A\u6708\u5F00\u59CB").addDropdown((dropdown) => {
      ...
      dropdown.setValue(String(this.plugin.settings.quarterStartMonth));
      dropdown.onChange(async (value) => {
        await this.plugin.updateSettings({ quarterStartMonth: parseInt(value) });
      });
    });
    const dailySection = this.createSection("\u65E5\u8BB0");
    new import_obsidian2.Setting(dailySection).setName("\u6807\u9898\u683C\u5F0F").setDesc("\u652F\u6301 YYYY\uFF08\u5E74\u4EFD\uFF09\u3001MM\uFF08\u6708\u4EFD\uFF09\u3001DD\uFF08\u65E5\u671F\uFF09").addText(
      (text) => text.setPlaceholder("YYYY-MM-DD").setValue(this.plugin.settings.dailyTitleFormat).onChange(async (value) => {
        await this.plugin.updateSettings({ dailyTitleFormat: value });
      })
    );
    new import_obsidian2.Setting(dailySection).setName("\u9ED8\u8BA4\u6587\u4EF6\u5939\u8DEF\u5F84").setDesc("\u521B\u5EFA\u65E5\u8BB0\u65F6\u7684\u9ED8\u8BA4\u4FDD\u5B58\u8DEF\u5F84\uFF08\u7559\u7A7A\u4E3A\u6839\u76EE\u5F55\uFF09").addText(
      (text) => text.setPlaceholder("\u4F8B\u5982: notes/\u65E5\u8BB0").setValue(this.plugin.settings.dailyFolderPath).onChange(async (value) => {
        await this.plugin.updateSettings({ dailyFolderPath: value });
      })
    );
    const weeklySection = this.createSection("\u5468\u8BB0");
    new import_obsidian2.Setting(weeklySection).setName("\u6807\u9898\u683C\u5F0F").setDesc("\u652F\u6301 YYYY\uFF08\u5E74\u4EFD\uFF09\u3001{week}\uFF08\u5468\u6570\uFF09\uFF0C\u4F8B\u5982\uFF1AYYYY-{week}\u5468").addText(
      (text) => text.setPlaceholder("YYYY-{week}\u5468").setValue(this.plugin.settings.weeklyTitleFormat).onChange(async (value) => {
        await this.plugin.updateSettings({ weeklyTitleFormat: value });
      })
    );
    new import_obsidian2.Setting(weeklySection).setName("\u9ED8\u8BA4\u6587\u4EF6\u5939\u8DEF\u5F84").setDesc("\u521B\u5EFA\u5468\u8BB0\u65F6\u7684\u9ED8\u8BA4\u4FDD\u5B58\u8DEF\u5F84\uFF08\u7559\u7A7A\u4E3A\u6839\u76EE\u5F55\uFF09").addText(
      (text) => text.setPlaceholder("\u4F8B\u5982: notes/\u5468\u8BB0").setValue(this.plugin.settings.weeklyFolderPath).onChange(async (value) => {
        await this.plugin.updateSettings({ weeklyFolderPath: value });
      })
    );
    const monthlySection = this.createSection("\u6708\u5EA6\u7B14\u8BB0");
    new import_obsidian2.Setting(monthlySection).setName("\u6807\u9898\u683C\u5F0F").setDesc("\u652F\u6301 YYYY\uFF08\u5E74\u4EFD\uFF09\u3001MM\uFF08\u6708\u4EFD\uFF09").addText(
      (text) => text.setPlaceholder("YYYY\u5E74MM\u6708").setValue(this.plugin.settings.monthlyTitleFormat).onChange(async (value) => {
        await this.plugin.updateSettings({ monthlyTitleFormat: value });
      })
    );
    new import_obsidian2.Setting(monthlySection).setName("\u9ED8\u8BA4\u6587\u4EF6\u5939\u8DEF\u5F84").setDesc("\u521B\u5EFA\u6708\u5EA6\u7B14\u8BB0\u65F6\u7684\u9ED8\u8BA4\u4FDD\u5B58\u8DEF\u5F84\uFF08\u7559\u7A7A\u4E3A\u6839\u76EE\u5F55\uFF09").addText(
      (text) => text.setPlaceholder("\u4F8B\u5982: notes/\u6708\u5EA6").setValue(this.plugin.settings.monthlyFolderPath).onChange(async (value) => {
        await this.plugin.updateSettings({ monthlyFolderPath: value });
      })
    );
    const quarterlyNotesSection = this.createSection("\u5B63\u5EA6\u7B14\u8BB0");
    new import_obsidian2.Setting(quarterlyNotesSection).setName("\u6807\u9898\u683C\u5F0F").setDesc("\u652F\u6301 YYYY\uFF08\u5E74\u4EFD\uFF09\u3001{quarter}\uFF08\u5B63\u5EA6\u540D\uFF09").addText(
      (text) => text.setPlaceholder("YYYY\u5E74-{quarter}\u5B63\u5EA6").setValue(this.plugin.settings.quarterlyTitleFormat).onChange(async (value) => {
        await this.plugin.updateSettings({ quarterlyTitleFormat: value });
      })
    );
    new import_obsidian2.Setting(quarterlyNotesSection).setName("\u9ED8\u8BA4\u6587\u4EF6\u5939\u8DEF\u5F84").setDesc("\u521B\u5EFA\u5B63\u5EA6\u7B14\u8BB0\u65F6\u7684\u9ED8\u8BA4\u4FDD\u5B58\u8DEF\u5F84\uFF08\u7559\u7A7A\u4E3A\u6839\u76EE\u5F55\uFF09").addText(
      (text) => text.setPlaceholder("\u4F8B\u5982: notes/\u5B63\u5EA6").setValue(this.plugin.settings.quarterlyFolderPath).onChange(async (value) => {
        await this.plugin.updateSettings({ quarterlyFolderPath: value });
      })
    );
    const yearlySection = this.createSection("\u5E74\u5EA6\u7B14\u8BB0");
    new import_obsidian2.Setting(yearlySection).setName("\u6807\u9898\u683C\u5F0F").setDesc("\u652F\u6301 YYYY\uFF08\u5E74\u4EFD\uFF09").addText(
      (text) => text.setPlaceholder("YYYY").setValue(this.plugin.settings.yearlyTitleFormat).onChange(async (value) => {
        await this.plugin.updateSettings({ yearlyTitleFormat: value });
      })
    );
    new import_obsidian2.Setting(yearlySection).setName("\u9ED8\u8BA4\u6587\u4EF6\u5939\u8DEF\u5F84").setDesc("\u521B\u5EFA\u5E74\u5EA6\u7B14\u8BB0\u65F6\u7684\u9ED8\u8BA4\u4FDD\u5B58\u8DEF\u5F84\uFF08\u7559\u7A7A\u4E3A\u6839\u76EE\u5F55\uFF09").addText(
      (text) => text.setPlaceholder("\u4F8B\u5982: notes/\u5E74\u5EA6").setValue(this.plugin.settings.yearlyFolderPath).onChange(async (value) => {
        await this.plugin.updateSettings({ yearlyFolderPath: value });
      })
    );
    const noteFolderSection = this.createSection("\u7B14\u8BB0\u626B\u63CF\u76EE\u5F55");
    new import_obsidian2.Setting(noteFolderSection).setName("\u626B\u63CF\u76EE\u5F55").setDesc("\u7559\u7A7A = \u626B\u63CF\u6574\u4E2A\u4ED3\u5E93\uFF08Plus: \u9ED8\u8BA4\u7EDF\u8BA1\u5168\u5E93\u6240\u6709\u6587\u4EF6\uFF0C\u542B\u9644\u4EF6\uFF09").addText(
      (text) => text.setPlaceholder("\u4F8B\u5982: notes/").setValue(this.plugin.settings.noteFolderPath).onChange(async (value) => {
...
var ChangelogModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    const { contentEl } = this;
    contentEl.empty();
    contentEl.addClass("kumoo-changelog");
    contentEl.createEl("h2", { text: "Kumoo Calendar \u66F4\u65B0\u65E5\u5FD7" });
    contentEl.createEl("div", {
      text: "\u4F5C\u8005\uFF1AKuMoo \xB7 \u57FA\u4E8E Note Calendar v1.3.1 (MIT)",
    ...
    for (const entry of CHANGELOG) {
      ...
      const head = contentEl.createEl("h3");
      head.textContent = `v${entry.version} \xB7 ${entry.date}`;
      if (isCurrent)
        head.textContent += "\uFF08\u5F53\u524D\u7248\u672C\uFF09";
